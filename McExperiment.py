...
def Experiments_car(MC, path_length, ret_df, CAP, SAVE, g, ratio, years=[60,84,120], tax=0.25):
    """
    Doing evaluations on all MC paths
    ...
    for mc in range(MC):
        ...
        mcdf = Path_Generator(path_length, ret_df) # Continue path
        # mcdf = Path_Generator_HMM(path_length) #HMM path
        temdf = MC_generate_car(mcdf,CAP,SAVE,g,ratio)
        temdf.reset_index(inplace=True,drop=True)
        # Loc out end of different years
        rst = temdf.loc[np.r_[years[0]-1,years[1]-1,years[2]-1],'cap_total']
        rst2 = temdf.loc[np.r_[years[0]-1,years[1]-1,years[2]-1],'cap_input'] 
...
def Experiments_dynamic(MC, path_length, ret_df, CAP, SAVE, g, years=[60,84,120], tax=0.25):
    """
    Doing evaluations on all MC paths
    ...
    for mc in range(MC):
        ...
        mcdf = Path_Generator(path_length, ret_df) # Continue path
        # mcdf = Path_Generator_HMM(path_length) #HMM path
        temdf = MC_generate_dynamic(mcdf,CAP,SAVE,g)
        temdf.reset_index(inplace=True,drop=True)
        # Loc out end of different years
        rst = temdf.loc[np.r_[years[0]-1,years[1]-1,years[2]-1],'cap_total']
        rst2 = temdf.loc[np.r_[years[0]-1,years[1]-1,years[2]-1],'cap_input'] 
